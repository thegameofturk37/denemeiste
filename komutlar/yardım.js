...
const sall = client.emojis.get("616084294091276316")
  ...
  if (args[0] === "özel") {
    ...
    let müzik = [
      `
[${prefix}oynat](Neptuneforum.cf/) • İstenilen şarkıyı oynatır
[${prefix}durdur](Neptuneforum.cf/) • Çalan şarkıyı durdurur
[${prefix}duraklat](Neptuneforum.cf/ • Çalan şarkıyı duraklatır
[${prefix}devam-ettir](Neptuneforum.cf/) • Duraklatılmış şarkıyı devam ettirir
[${prefix}ses](Neptuneforum.cf/) • Şarkının sesini ayarlamanızı sağlar
[${prefix}geç](Neptuneforum.cf/) • Sıradaki şarkıya geçer
[${prefix}kuyruk](Neptuneforum.cf/) • Şarkı kuyruğunu ve çalan şarkıyı gösterir  
`
    ];
  ...
  let cats = arr
    ...
    .map(
      k =>
        `[${prefix}yardım ${k}](Neptuneforum.cf/) • ${k.charAt(0).toUpperCase() +
...
*/
    const embed = new Discord.RichEmbed()
      ...
      .setDescription(
        `${cats}
        [${prefix}yardım müzik](Neptuneforum.cf/) • Müzik komutlarını gösterir`
      )
      .setFooter(`Örnek kullanım: ${prefix}yardım eğlence`)
    .addField(`Linkler`, `[Bot Davet Linki](hhttps://discordapp.com/oauth2/authorize?client_id=485110852739923969&scope=bot&permissions=268443710) • [Destek Sunucusu](https://discord.gg/AJ6tpFT) • [İnternet Sitesi](http://Neptuneforum.cf/)`);
    ...
    const e = new Discord.RichEmbed()
      ...
      .setDescription(
        list
          .map(
            k =>
              `[${prefix + k.help.name}](Neptuneforum.cf/) • ${k.help.description}`
          )
          .join("\n")
      );
    msg.channel.send(e);
